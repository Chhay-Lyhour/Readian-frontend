...
function RemoveUserPopup({ user, reason, setReason, onConfirm, onAbort }) {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-sm">
        <h2 className="text-2xl font-bold text-center mb-4">Are you sure?</h2>
        <p className="text-center mb-4">
          You are about to remove user <strong>{user.username}</strong>. 
        ...
        <div className="flex justify-between">
          <button 
            onClick={onConfirm}
            className="bg-red-500 text-[#FFD7DF] font-bold py-2 px-6 rounded-lg hover:bg-[#FFD7DF] hover:text-red-500 transition-all duration-300"
          >
            Confirm
          </button>
          <button 
